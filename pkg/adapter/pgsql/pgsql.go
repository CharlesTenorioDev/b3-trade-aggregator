...
func New(conf *config.Config) *dabase_pool {
	...
	if SRV_DB_HOST != "" {
		...
		logger.Info("A variável SRV_DB_HOST é obrigatória!")
		os.Exit(1)
	}
	...
	if SRV_DB_USER != "" {
		...
		logger.Info("A variável SRV_DB_USER é obrigatória!")
		os.Exit(1)
	}
	...
	if SRV_DB_PASS != "" {
		...
		logger.Info("A variável SRV_DB_PASS é obrigatória!")
		os.Exit(1)
	}
	...
	if SRV_DB_NAME != "" {
		...
		logger.Info("A variável SRV_DB_NAME é obrigatória!")
		os.Exit(1)
	}
...
func (d *dabase_pool) CloseConnection() error {
	if err := d.conn.Close(); err != nil {
		logger.Error("Erro to Connect in POSTGRESQL", err)
		return err
	}
